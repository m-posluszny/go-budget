...
func MockValidateCredentials(creds Credentials, mock Credentials, t *testing.T) {
...
func TestGetUserFromUid(t *testing.T) {
	...
	mockCreds := Credentials{Uid: "a-b-c-d", Username: "abcd", PasswordHash: []byte{1, 2, 3}}
	...
	creds, err := GetUserFromUid(db, mockCreds.Uid)
	if err != nil {
		fmt.Println(err)
